...
async function fetchArchivedData() {
  try {
      ...
      console.log('Fetched archived data:', JSON.stringify(archivedData, null, 2));

      // Directly pass the archived data array to populate the table
      if (Array.isArray(archivedData)) {
          populateHistoryTable(archivedData); // Populate the table with the archived data
      } else {
          console.error('Unexpected data format:', archivedData);
      }
  } catch (error) {
...
function populateHistoryTable(archivedData) {
  const historyTableBody = document.querySelector('#customersTableBody');
  historyTableBody.innerHTML = ''; // Clear the table body before adding new rows

  // Loop through the archived data and populate the table
  archivedData.forEach(customer => {
      ...
      const row = `
          <tr>
              ...
              <td class="truncate" title="${customer.loggedInUserName}">${customer.loggedInUserName}</td> <!-- Agent column -->
              <td>
                  <!-- Optional actions like Show More or other buttons -->
...
// Fetch archived data when the page loads
fetchArchivedData();

// Optional: Add a refresh button to reload archived data
document.getElementById('refreshUsersButton').addEventListener('click', fetchArchivedData);
